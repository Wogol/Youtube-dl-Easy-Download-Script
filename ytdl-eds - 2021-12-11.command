    ...
    echo -e "\n\n*** - Starting download thumbnail, json, subtitle and information file:\n"

    yt-dlp --skip-download --write-thumbnail --write-info-json --all-subs --restrict-filenames -o "$directory%(title)s/%(title)s.%(ext)s" $url



    # - THE INFORMATION FILE -

    textfile=$(yt-dlp --get-filename --restrict-filenames -o "$directory%(title)s/%(title)s.txt" $url)
    ...
    yt-dlp $av --restrict-filenames -o "$directory%(title)s/%(title)s.%(ext)s" $url
